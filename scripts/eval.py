...
# def compute_statistics_all_tokens(model: torch.nn.Module, batch: dict, pad_value: int = 0, **kwargs) -> dict:
...
#     token_logprob = cross_entropy(logits.permute(0, 2, 1), labels, reduction="none", ignore_index=pad_value).neg()

#     out["token_logprob"] = token_logprob.cpu().numpy().tolist()
#     out["token_ids"] = labels.cpu().numpy().tolist()
#     return out
